...
export class WorkerService {
  ...
  async create(createWorkerDto: CreateWorkerDto) {
    ...
    if (!isSpeciality) {
      throw new NotFoundException('Specified speciality does not exist');
    }

    const blockIDs = createWorkerDto.block_IDs.flat();

    let blocks = await this.blockModel.find({ _id: { $in: blockIDs } });
    if (blocks.length !== blockIDs.length) {
      throw new NotFoundException('One or more block IDs do not exist');
    }
    ...
    await Promise.all(
      blocks.map(async (block) => {
        block.workers.push(newWorker);
        return block.save();
      }),
    );

    await Promise.all(
      blocks.map(async (block) => newWorker.blocks.push(block)),
    );

    isSpeciality.worker.push(newWorker);
    await isSpeciality.save();
  ...
  async update(id: ObjectId, updateWorkerDto: UpdateWorkerDto) {
    const { phone_number } = updateWorkerDto;
